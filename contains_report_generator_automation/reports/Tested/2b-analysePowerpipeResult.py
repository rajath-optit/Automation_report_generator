...
def create_simplified_report_with_pivot(report_file, final_report_file):
    ...
    print(f"Report with pivot table saved as {final_report_file}")

def main():
    report_file = input("Enter the report file name (e.g., input.csv): ").strip()
    base_name = os.path.splitext(os.path.basename(report_file))[0]
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

    # Improved file names
    simplified_report_file = f"{base_name}_compliant_non_compliant_report_{timestamp}.xlsx"
    pivot_report_file = f"{base_name}_detailed_report_with_pivot_{timestamp}.xlsx"

    try:
        create_simplified_report(report_file, simplified_report_file)
        create_simplified_report_with_pivot(report_file, pivot_report_file)
